...
$pdo->exec("TRUNCATE TABLE flight_logs");
...
for ($i = 0; $i < 60; $i++) {
    ...
    $stmt->execute([
        ...
        $duration, $cabin, $price, $stops
    ]);

    $seed_sql_content .= "INSERT INTO flights (flight_number, origin, destination, departure_time, arrival_time, duration, cabin_class, price, stops) VALUES ('$flight_no', '$origin', '$dest', '{$departure->format('Y-m-d H:i:s')}', '{$arrival->format('Y-m-d H:i:s')}', $duration, '$cabin', $price, $stops);\n";
}
...
for ($i = 0; $i < 200; $i++) {
    ...
    $stmt->execute([
        $flight_no, $route, $time->format('Y-m-d H:i:s'), $status, $price, $type
    ]);

    $seed_sql_content .= "INSERT INTO flight_logs (flight_number, route, scheduled_time, status, price, type) VALUES ('$flight_no', '$route', '{$time->format('Y-m-d H:i:s')}', '$status', $price, '$type');\n";
}
